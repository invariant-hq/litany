(* Vendored from _ref/dune/otherlibs/ocamlc-loc/src/ocamlc_loc.ml with the
   Dyn-dependent printers stripped (they were the only use of the [dyn]
   library); parsing logic is byte-identical to dune's. *)
include Lexer
...
module Tokens : sig
  ...
  type t = { lexbuf : Lexing.lexbuf; mutable unread : Lexer.token list }
