...
let p1 x = if p x then true else false (* FIRE *)
let p2 ok = if ok then false else true (* FIRE *)
...
(* Argument position: the parenthesized if's location includes the
   ...
   unsafe golden). *)
let p3 x = string_of_bool (if p x then true else false) (* FIRE *)
let p4 ok = string_of_bool (if ok then false else true) (* FIRE *)
...
let cor02 c =
  let not _ = 0 in
  ignore (not 1);
  if c then false else true (* FIRE *)
