...
let p1 = if o = None then default else run o (* FIRE *)
let p2 = o <> None (* FIRE *)
let p3 = None = lookup 1 (* FIRE *)

(* A computed operand parenthesizes in the fix. *)
let p4 = lookup 1 <> None (* FIRE *)
...
(* The tag decides without traversing the payload: a Some operand still
   ...
   fix is safe. *)
let p5 = Some (run o) = None (* FIRE *)

(* The spec's loop context: the comparison is just an expression. *)
let drain q =
  while Queue.peek_opt q <> None (* FIRE *) do
    Queue.clear q
  done
...
let p_begin = check begin o = None end (* FIRE *) [@@ocamlformat "disable"]

(* A multi-line chain anchors at the chain start. *)
let c5 the_first_compared_operand the_second_compared_operand =
  the_first_compared_operand <> None (* FIRE *)
  && the_second_compared_operand <> None
...
let n9 first_compared_operand second_compared_operand =
  let ( && ) x y = x || y in
  first_compared_operand = None (* FIRE *)
  && second_compared_operand = None (* FIRE *)
