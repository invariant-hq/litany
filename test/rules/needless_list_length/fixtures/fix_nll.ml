...
module G (L : module type of List) = struct
  let is_empty ys = L.length ys = 0 (* FIRE *)
end
...
let expected_empty ys = List.length ys = 0
...
let prefix_spelling = ( = ) (List.length xs) 0 (* FIRE *)
...
let alias_input (v : t) = List.length v = 0 (* FIRE *)
...
let paren_ctx () = assert (List.length xs = 0) (* FIRE *)
...
(* The same, under a prefix application and as a cons head: the restored
   ...
   the cell. *)
let paren_not () = not (List.length xs = 0) (* FIRE *)
let paren_cons () = (List.length xs = 0) (* FIRE *) :: []
