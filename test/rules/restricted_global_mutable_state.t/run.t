...
executables own their process.
  ...
  File "lib/statelib.ml", line 1, characters 4-11:
  1 | let counter = ref 0
          ^^^^^^^
  Warning 0 [restricted-global-mutable-state]: toplevel mutable state in a library is a process-wide global; create it in the caller and pass it down
    
  File "lib/statelib.ml", line 2, characters 4-12:
  2 | let registry : (string, int) Hashtbl.t = Hashtbl.create 16
          ^^^^^^^^
  Warning 0 [restricted-global-mutable-state]: toplevel mutable state in a library is a process-wide global; create it in the caller and pass it down
    
  File "lib/statelib.ml", line 6, characters 4-9:
  6 | let stats = { hits = 0 }
          ^^^^^
  Warning 0 [restricted-global-mutable-state]: toplevel mutable state in a library is a process-wide global; create it in the caller and pass it down
