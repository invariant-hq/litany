...
let p1 b i = if i >= 10 then 0 else match b with true -> 1 | false -> 2 (* FIRE *)
...
let p2 a d =
  if d then 0
    ...
    match a (* FIRE *) with
    | false ->
        Format.printf "m %d" 42;
        1
    | true -> 2

(* Arms continuing the cascade: a standalone match whose false arm is
   itself an if/else. *)
let p3 a d = match a with true -> 1 | false -> if d then 2 else 3 (* FIRE *)
...
let p5 a d = succ (match a with true -> 1 | false -> if d then 2 else 3) (* FIRE *)

(* The else-continuation shape, parenthesized: the pair is the author's
   and stays. *)
let p6 b i = if i >= 10 then 0 else (match b with true -> 1 | false -> 2) (* FIRE *)
