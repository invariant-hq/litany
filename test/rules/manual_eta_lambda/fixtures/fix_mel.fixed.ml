...
let p1 = Result.map parse r (* FIRE *)

(* Under List.map. *)
let p2 = List.map step xs (* FIRE *)

(* In a pipeline. *)
let p3 = xs |> List.map step (* FIRE *)

(* Two parameters, in order. *)
let p4 = List.fold_left add 0 xs (* FIRE *)

(* A module-path callee. *)
let p5 = List.map String.length [ "a"; "bb" ] (* FIRE *)

(* Trailing-open, unparenthesized: the replacement is atomic and the
   slice had no delimiters, so none are added. *)
let p6 = xs |> List.length (* FIRE *)

(* The callee's own arity is irrelevant: [add x] is [add] at the same
   type. *)
let p7 = List.map add xs (* FIRE *)
...
let p8 = [ step ] (* FIRE *)
let p9 = step 5 (* FIRE *)
let p10 = tagged ~tag:0 step (* FIRE *)
...
let rec size t =
  match t with
  | Leaf -> 1
  | Node ts -> sum (List.map size ts) (* FIRE *)
