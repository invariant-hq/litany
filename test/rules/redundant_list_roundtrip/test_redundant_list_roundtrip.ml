...
let rule = Litany_rules.Redundant_list_roundtrip.rule
let source = "fixtures/fix_rlr.ml"
let cmt = "fixtures/.fix_rlr.objs/byte/fix_rlr.cmt"

let () =
  Windtrap.run "redundant-list-roundtrip"
    [
      test "declares its one metadata record" (fun () ->
          equal string "redundant-list-roundtrip" (Rule.name rule);
