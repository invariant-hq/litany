(* Apply-argument compat, 5.3 leg: [Texp_apply] arguments are
   [(arg_label * expression option) list]. Selected by a copy rule in [dune];
   5.4 moved to [(arg_label * apply_arg) list] (see apply_arg_54.ml).
...
let unlabeled_all args =
  let rec go acc = function
    | [] -> Some (List.rev acc)
    | (Asttypes.Nolabel, Some (e : Typedtree.expression)) :: rest ->
        go (e :: acc) rest
    | _ -> None
  in
  go [] args
