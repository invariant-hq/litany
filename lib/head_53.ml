(* Description-head compat, 5.3 leg: [constructor_description] and
   [label_description] live in [Types] (5.4 moved both to [Data_types]; see
   head_54.ml). Selected by a copy rule in [dune] — the same
...
let cstr (cd : Types.constructor_description) =
...
let cstr_ext (cd : Types.constructor_description) =
...
(* [lbl ld] is [cstr] for label descriptions: the head path of the label's
   record type. *)
let lbl (ld : Types.label_description) =
...
(* [package pt] is the package type's path and its spelled name — the
   ...
   [pack_txt] here; [tpt_path]/[tpt_txt] from 5.4), which is why the read
   lives in this seam. *)
let package (pt : Typedtree.package_type) =
  (pt.Typedtree.pack_path, pt.Typedtree.pack_txt)
